...
###Поддержка половинной точности FP16 для видеокарт с тензорными ядрами. 
Для повышения метрик производительности инференса моделей, работающая через обретку
torch.cuda.amp.autocast(). Включается через параметр --fp16 перед запуском eval.py. Содержится в ./improvements/FP16_integration
###Плавное смешение изображений (MixUp). 
Аугментация для повышение обобщающей способности модели и предотвращения переобучения. Работает по принципу 
